...
import "./friends.css";
import { useState, useEffect } from "react";
import { makeRequest } from "../../axios"; // ✅ Axios instance with token

const Friends = () => {
  const [counts, setCounts] = useState({ followers: 0, following: 0 });
  const [allUsers, setAllUsers] = useState([]); // ✅ Store all users
  const [filteredUsers, setFilteredUsers] = useState([]); // ✅ Store filtered users
  const [followedUsers, setFollowedUsers] = useState(new Set()); // ✅ Store followed users
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState(""); // ✅ Search term state
  ...
  const fetchAllUsers = async () => {
    try {
      const res = await makeRequest.get("/relationships/allUsers");
      if (res.data && Array.isArray(res.data)) {
        setAllUsers(res.data);
  ...
  // ✅ Fetch Followed Users for Button State
  const fetchFollowedUsers = async () => {
    try {
