    ...
    <div className="stories-container">
      <div className="stories-wrapper">
        ...
        {error ? (
          ...
          <div className="stories-scroll-container">
            {data.map((story, index) => (
  <div key={story._id} className="story-slide" onClick={() => openStoryView(index)}>
    {isVideo(story.img) ? (
      <video className="story-media" muted autoPlay loop>
        <source src={story.img} type="video/mp4" />
        Your browser does not support the video tag.
      </video>
    ) : (
    ...
    {String(story.userId?._id) === String(currentUser.id) && (
      <button
        className="delete-btn"
        onClick={(e) => handleDeleteStory(story._id, e)}
      >
        ❌
      </button>
    )}
      ...
      {viewingStory && (
        <div className="story-view-overlay" onClick={closeStoryView}>
          <div className="story-view-container" onClick={(e) => e.stopPropagation()}>
            <div className="story-view-header">
              <div className="story-user-info">
                <img src={currentUser.profilePic} alt="User" className="story-user-pic" />
                <span>{viewingStory.name}</span>
              </div>
              <button className="close-btn" onClick={closeStoryView}>×</button>
            </div>
            ...
            <div className="story-content">
              {isVideo(viewingStory.img) ? (
                <video controls autoPlay className="story-view-media">
                  <source src={viewingStory.img} type="video/mp4" />
                  Your browser does not support the video tag.
                </video>
              ) : (
                <img src={viewingStory.img} alt="story" className="story-view-media" />
              )}
              ...
              <button className="story-nav-btn next-btn" onClick={goToNextStory}>
                ▶
              </button>
            </div>
